# =============================================================================
# shm-next — Integration Tests: SpoolTask Repository
# =============================================================================
...
class TestSpoolTaskRepository:
    ...
    async def test_create_task(self, db_session):
        ...
        task_id = await repo.create_task(action_type="send_notification", payload={"user_id": "123", "message": "Hello"}, priority=100, max_retries=3)
    ...
    async def test_move_to_dlq(self, db_session):
        repo = SpoolTaskRepository(db_session)
        task_id = await repo.create_task(action_type="send_notification", payload={})
        result = await repo.move_to_dlq(task_id, "Permanent failure")
        assert result is True

    async def test_spool_task_lifecycle(self, db_session):
        repo = SpoolTaskRepository(db_session)
        task_id = await repo.create_task(action_type="webhook_call", payload={"url": "https://example.com/hook"}, priority=10)
        processing = await repo.mark_processing(task_id, "worker-1")
        assert processing is True
        completed = await repo.mark_completed(task_id, {"response": "ok"})
