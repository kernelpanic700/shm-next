...
export default function TariffsPage() {
  ...
  const columns = useMemo<ColumnDef<Tariff>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <span className="font-medium">{price.toLocaleString(numberLocale, { minimumFractionDigits: 2 })} {currency}</span>;
      },
    },
    {
      ...
      cell: ({ row, table }) => {
        ...
        return (
          <div className="flex gap-1">
            <Button variant="ghost" size="sm" onClick={() => (table.options.meta as any)?.onEdit(tariff)} title={t('Edit')}>
              <Edit className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" onClick={() => (table.options.meta as any)?.onDeactivate(tariff)} title={t('Deactivate')}>
  ...
  if (isError) {
    return (
      <div className="flex-1 space-y-4 p-8 pt-6">
        <h2 className="text-3xl font-bold tracking-tight">{t('Tariffs')}</h2>
        <p className="text-destructive">{t('TariffsLoadError')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 space-y-4 p-8 pt-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('TariffsList')}</CardTitle>
          <CardDescription>{t('TariffsListDescription')}</CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable 
            columns={columns} 
